...
namespace HighlightPlus2D {
    ...
    public partial class HighlightEffect2D : MonoBehaviour {
                ...
                if (glow > 0) {
                    for (int k = 0; k < rms.Length; k++) {
                        ...
                        if (useGPUInstancing) {
                            ...
                            DrawSubMeshesInstanced(quadMesh, rms[k].renderer, rms[k].fxMatGlow, propMatrices, glowProps);
                        } else {
                            // Non instanced glow
                            for (int j = 0; j < glowPasses.Length; j++) {
                    ...
                    for (int k = 0; k < rms.Length; k++) {
                        ...
                        if (outlineQuality != QualityLevel.Simple) {
                            ...
                            if (useGPUInstancing) {
                                ...
                                DrawSubMeshesInstanced(quadMesh, rms[k].renderer, rms[k].fxMatOutline, propMatrices, outlineProps);
                            } else {
                                for (int z = 0; z < offsets.Length; z++) {
                                    Vector2 newPivot = new Vector2(originalPivotPos.x + disp.x * offsets[z].x, originalPivotPos.y + disp.y * offsets[z].y);
        ...
        void DrawSubMeshesInstanced(Mesh mesh, Renderer renderer, Material mat, List<Matrix4x4> matrices, MaterialPropertyBlock props) {
            int subMeshCount = mesh.subMeshCount;
            if (subMeshCount == 1) {
                Graphics.DrawMeshInstanced(mesh, 0, mat, matrices, props);
                return;
            }
            ...
            for (int k = 0; k < subMeshCount; k++) {
                ...
                Graphics.DrawMeshInstanced(mesh, k, mat, matrices, props);
            }
        }
